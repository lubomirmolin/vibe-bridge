use super::mapping::{
    filter_hidden_timeline_entries_and_extract_pending_input, map_thread_snapshot,
    map_thread_summary,
};
use super::models::fetch_model_catalog;
use super::*;
...
impl CodexGateway {
    ...
    pub(super) fn start_codex_turn_streaming<F, G, H, I>(
        &self,
        thread_id: &str,
    ...
    pub(super) async fn interrupt_codex_turn(
        &self,
        thread_id: &str,
        turn_id: &str,
    ) -> Result<GatewayTurnMutation, String> {
    ...
    pub(super) async fn resolve_codex_active_turn_id(
        &self,
        thread_id: &str,
...
pub(super) fn extract_generated_thread_title(agent_message: Option<&str>) -> Option<String> {
...
pub(super) fn normalize_generated_thread_title(raw_title: &str) -> Option<String> {
    ...
    let trimmed = normalized_whitespace
        ...
        .trim();
    if trimmed.is_empty()
        || trimmed.eq_ignore_ascii_case("untitled thread")
        || trimmed.eq_ignore_ascii_case("new thread")
        || trimmed.eq_ignore_ascii_case("fresh session")
    {
        return None;
    }
    ...
    if normalized.is_empty()
        || normalized.eq_ignore_ascii_case("untitled thread")
        || normalized.eq_ignore_ascii_case("new thread")
        || normalized.eq_ignore_ascii_case("fresh session")
    {
        return None;
    }
