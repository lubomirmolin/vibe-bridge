use super::codex::fetch_thread_snapshot_from_archive;
use super::*;
...
impl CodexGateway {
    ...
    pub(super) fn interrupt_claude_turn(
        &self,
        thread_id: &str,
    ) -> Result<GatewayTurnMutation, String> {
        let thread_id = thread_id.to_string();
    ...
    pub(super) fn resolve_active_claude_turn_id(&self, thread_id: &str) -> Result<String, String> {
        let thread_id = thread_id.to_string();
        self.active_claude_processes
            .lock()
            .expect("active claude process lock should not be poisoned")
...
fn detect_git_context(workspace: &str) -> WorkspaceGitContext {
    let repository = run_git_output(workspace, ["rev-parse", "--show-toplevel"])
        .ok()
        .as_deref()
        .and_then(super::mapping::derive_repository_name_from_cwd)
        .or_else(|| super::mapping::derive_repository_name_from_cwd(workspace))
