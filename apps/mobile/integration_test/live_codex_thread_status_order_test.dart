...
void main() {
  ...
  testWidgets(
    'real bridge Codex thread keeps the running state until README review activity settles',
    (tester) async {
      ...
      await _assertNoPrematureNonRunningState(
        tester,
        bridgeApiBaseUrl: bridgeApiBaseUrl,
        threadId: createdThreadId,
      );
      ...
      debugPrint(
        'LIVE_CODEX_STATUS_ORDER_RESULT '
...
String _buildReadmeReviewPrompt() {
  return 'What do you think about our readme? '
      'I wanted it to look great for open source project.';
}
...
Future<void> _requireAndroidLoopbackDevice(String bridgeApiBaseUrl) async {
  if (!Platform.isAndroid) {
    fail(
      ...
      'Run it with `flutter drive --driver=test_driver/integration_test.dart --target=integration_test/live_codex_thread_status_order_test.dart -d <android-device-id>`.',
    );
  }
...
Future<void> _assertNoPrematureNonRunningState(
  WidgetTester tester, {
  required String bridgeApiBaseUrl,
  required String threadId,
  Duration timeout = const Duration(minutes: 2),
}) async {
  final expectedPrompt = _buildReadmeReviewPrompt();
  final args = ThreadDetailControllerArgs(
    bridgeApiBaseUrl: bridgeApiBaseUrl,
    threadId: threadId,
  );
  ...
  final deadline = DateTime.now().add(timeout);
  var lastSnapshotPollAt = DateTime.fromMillisecondsSinceEpoch(0);
  var lastEntryGrowthAt = DateTime.now();
  var maxSnapshotEntryCount = 0;
  var sawRunningState = false;
  var sawMeaningfulLiveActivityWhileRunning = false;
  String? firstNonRunningStatus;
  int? firstNonRunningEntryCount;
  final statusLog = <String>[];

  while (DateTime.now().isBefore(deadline)) {
    await tester.pump(const Duration(milliseconds: 150));
    final state = container.read(threadDetailControllerProvider(args));
    final runningIndicatorVisible = find
        .byKey(const Key('thread-running-indicator-card'))
        .evaluate()
        .isNotEmpty;
    ...
    if (DateTime.now().difference(lastSnapshotPollAt) <
        const Duration(milliseconds: 350)) {
      continue;
    }
    lastSnapshotPollAt = DateTime.now();
    ...
    final entryCount =
        (snapshot['entries'] as List<dynamic>? ?? const []).length;
    statusLog.add('$rawStatus@$entryCount');
    if (entryCount > maxSnapshotEntryCount) {
      maxSnapshotEntryCount = entryCount;
      lastEntryGrowthAt = DateTime.now();
      if (firstNonRunningStatus != null &&
          entryCount > (firstNonRunningEntryCount ?? 0)) {
        fail(
          'Thread $threadId received $entryCount snapshot entries after status '
          '$firstNonRunningStatus first appeared. '
          'Observed statuses: ${statusLog.join(' -> ')}',
        );
      }
    }
    ...
    if (rawStatus == ThreadStatus.running.wireValue) {
      sawRunningState = true;
    } else if (sawRunningState && firstNonRunningStatus == null) {
      firstNonRunningStatus = rawStatus;
      firstNonRunningEntryCount = entryCount;
    }

    if (rawStatus == ThreadStatus.failed.wireValue) {
      fail('Codex thread $threadId failed during README review.');
    }
    if (rawStatus == ThreadStatus.interrupted.wireValue) {
      fail('Codex thread $threadId was interrupted during README review.');
    }
    if (snapshot['pending_user_input'] != null) {
      fail(
        'Codex thread $threadId unexpectedly requested user input during README review.',
      );
    }

    final hasMeaningfulActivity = state.items.any((item) {
      return switch (item.type) {
        ThreadActivityItemType.assistantOutput => item.body.trim().isNotEmpty,
    ...
    if (rawStatus == ThreadStatus.running.wireValue &&
        (hasMeaningfulActivity || entryCount > 2)) {
      sawMeaningfulLiveActivityWhileRunning = true;
    }
    final quietFor = DateTime.now().difference(lastEntryGrowthAt);
    if (sawRunningState &&
        !runningIndicatorVisible &&
        rawStatus.isNotEmpty &&
        rawStatus != ThreadStatus.running.wireValue &&
        hasMeaningfulActivity &&
        quietFor >= const Duration(seconds: 3)) {
      if (!sawMeaningfulLiveActivityWhileRunning) {
        fail(
          'Thread $threadId completed without surfacing meaningful live '
          'activity while still running. Observed statuses: ${statusLog.join(' -> ')}',
        );
      }
      await _assertSingleUserPromptInSnapshot(
        bridgeApiBaseUrl: bridgeApiBaseUrl,
        threadId: threadId,
  ...
  fail(
    'Timed out waiting for README review thread $threadId to settle without '
    'premature completion. Observed statuses: ${statusLog.join(' -> ')}',
  );
}

Future<void> _assertSingleUserPromptInSnapshot({
  required String bridgeApiBaseUrl,
  required String threadId,
  ...
  final entries = snapshot['entries'] as List<dynamic>? ?? const [];
  final matchingUserPrompts = entries.where((entry) {
    ...
    final text =
        ((payload['text'] ?? payload['delta'] ?? payload['message']) as String?)
            ?.trim();
    return role == 'user' && text == expectedPrompt;
  }).length;
  ...
  expect(
    ...
    1,
    reason:
        'Expected exactly one visible user prompt for $threadId in the final '
        'snapshot, but found $matchingUserPrompts.',
  );
}
