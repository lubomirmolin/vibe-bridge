...
class AnimatedBridgeBackground extends StatefulWidget {
  ...
  @override
  State<AnimatedBridgeBackground> createState() => _AnimatedBridgeBackgroundState();
}

class _AnimatedBridgeBackgroundState extends State<AnimatedBridgeBackground> with SingleTickerProviderStateMixin {
  late Ticker _ticker;
...
class _StarsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      paint.color = Colors.white.withOpacity(rand.nextDouble() * 0.3 + 0.1);
...
class _BridgePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var tx in towers) {
      ...
      canvas.drawLine(Offset(tx - 25, 100), Offset(tx + 25, 100), solidPaint..strokeWidth = 4..color = Colors.white.withOpacity(0.5));
      canvas.drawLine(Offset(tx - 22, 115), Offset(tx + 22, 115), solidPaint..strokeWidth = 2);
      canvas.drawLine(Offset(tx - 18, 130), Offset(tx + 18, 130), solidPaint..strokeWidth = 1);
      ...
      for (int i = 0; i < bracingY.length; i++) {
        ...
        canvas.drawLine(Offset(tx - widthAtY, cy), Offset(tx + widthAtY, cy), solidPaint..strokeWidth = 3..color = Colors.white.withOpacity(0.4));
        canvas.drawLine(Offset(tx - widthAtY, cy + 10), Offset(tx + widthAtY, cy + 10), solidPaint..strokeWidth = 1);
        
        if (i < bracingY.length - 1) {
          final nextCy = bracingY[i + 1];
          final nextWidth = 20 + ((nextCy - 100) / 600) * 15;
          canvas.drawLine(Offset(tx - widthAtY, cy), Offset(tx + nextWidth, nextCy), solidPaint..strokeWidth = 1..color = Colors.white.withOpacity(0.3));
          canvas.drawLine(Offset(tx + widthAtY, cy), Offset(tx - nextWidth, nextCy), solidPaint..strokeWidth = 1..color = Colors.white.withOpacity(0.3));
        }
      }
    }
    ...
    // Deck
    canvas.drawLine(const Offset(-200, 610), const Offset(1200, 610), deckPaint);
    ...
    canvas.drawPath(dashedCable, Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3
    ...
    for (int i = 0; i < 3; i++) {
      ...
      for (double x = xOffset; x < 1200; x += 42) {
        canvas.drawLine(Offset(x, 585.0 + i * 5), Offset(x + 2, 585.0 + i * 5), streamPaint);
      }
    }
